...
export class TodosService {
  private tasks: TaskAppearance[] = localStorage.getItem('tasks')
    ? JSON.parse(localStorage.getItem('tasks'))
    : [];

  private stream$: BehaviorSubject<TaskAppearance[]> = new BehaviorSubject<
    TaskAppearance[]
  >(this.tasks);
  ...
  constructor(
    ...
    if (
      this.tasks.length == 0 &&
      localStorage.getItem('basickElementOnce') == undefined
    ) {
      http
        .get<TaskAppearance[]>(
  ...
  sortDate(): void {
    if (
      this.tasks[0].id.fullDate < this.tasks[this.tasks.length - 1].id.fullDate
    ) {
